...
namespace Zobrist {
  extern Key psq[VARIANT_NB][PIECE_NB][SQUARE_NB];
}
...
static uint64 calc_key(Position& pos, int mirror)
{
  ...
  for (pt = PAWN; pt <= KING; ++pt)
    for (i = popcount(pos.pieces(color, pt)); i > 0; i--)
      key ^= Zobrist::psq[pos.variant()][make_piece(WHITE, pt)][i - 1];
  color = ~color;
  for (pt = PAWN; pt <= KING; ++pt)
    for (i = popcount(pos.pieces(color, pt)); i > 0; i--)
      key ^= Zobrist::psq[pos.variant()][make_piece(BLACK, pt)][i - 1];
...
static uint64 calc_key_from_pcs(int *pcs, int mirror)
{
  ...
  for (pt = PAWN; pt <= KING; ++pt)
    for (i = 0; i < pcs[color + pt]; i++)
      key ^= Zobrist::psq[CHESS_VARIANT][make_piece(WHITE, pt)][i];
  color ^= 8;
  for (pt = PAWN; pt <= KING; ++pt)
    for (i = 0; i < pcs[color + pt]; i++)
      key ^= Zobrist::psq[CHESS_VARIANT][make_piece(BLACK, pt)][i];
...
static int probe_wdl_table(Position& pos, int *success)
{
  ...
  // Test for KvK.
  if (key == (Zobrist::psq[pos.variant()][W_KING][0] ^ Zobrist::psq[pos.variant()][B_KING][0]))
    return 0;
