  ...
  void id_loop(Position& pos) {
    ...
    while (++depth <= MAX_PLY && !Signals.stop && (!Limits.depth || depth <= Limits.depth))
    {
        ...
        for (PVIdx = 0; PVIdx < PVSize; PVIdx++)
        {
            ...
            if (depth >= 5)
            {
                alpha = std::max(RootMoves[PVIdx].prevScore - Value(16),-VALUE_INFINITE);
                beta  = std::min(RootMoves[PVIdx].prevScore + Value(16), VALUE_INFINITE);
            }
            ...
            while (true)
            {
                ...
                if (bestValue <= alpha)
                {
                    alpha = std::max(bestValue - Value(24), -VALUE_INFINITE);

                    Signals.failedLowAtRoot = true;
                    Signals.stopOnPonderhit = false;
                }
                else if (bestValue >= beta)
                    beta = std::min(bestValue + Value(24), VALUE_INFINITE);
