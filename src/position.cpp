...
*/
        ...
        && ((ss >> row) && (sideToMove ? row == '3' : row == '6')))
  {
      ...
      if (   !(attackers_to(st->epSquare) & pieces(sideToMove, PAWN))
          ...
          st->epSquare = SQ_NONE;
#ifdef ATOMIC
      else if (is_atomic() && (DistanceRingBB[st->epSquare][1] & square<KING>(sideToMove)))
          st->epSquare = SQ_NONE;
#endif
  }
  else
...
bool Position::legal(Move m) const {
  ...
  if (is_atomic())
  {
      ...
      assert(!capture(m) || !(DistanceRingBB[to][1] & ksq));
      if (type_of(piece_on(from)) != KING)
      {
          if (DistanceRingBB[square<KING>(~us)][1] & ksq)
              return true;
          if (capture(m))
          {
              Square capsq = type_of(m) == ENPASSANT ? make_square(file_of(to), rank_of(from)) : to;
              Bitboard blast = DistanceRingBB[to][1] & (pieces() ^ pieces(PAWN));
      ...
      else if (DistanceRingBB[square<KING>(~us)][1] & to)
          return true;
  }
#endif
  ...
  if (type_of(m) == CASTLING)
  {
      ...
      for (Square s = to; s != from; s += step)
          ...
          if (is_atomic())
          {
              ...
              if (   !(DistanceRingBB[square<KING>(~us)][1] & s)
      ...
      return   !chess960
#ifdef ATOMIC
            ||  (is_atomic() && (DistanceRingBB[square<KING>(~us)][1] & to))
#endif
  ...
  {
#ifdef ATOMIC
      if (is_atomic() && kings_adjacent() && !(DistanceRingBB[square<KING>(~us)][1] & to))
      {
...
bool Position::pseudo_legal(const Move m) const {
  ...
  if (is_atomic())
  {
      ...
      if (capture(m))
      {
          ...
          if ((pieces(us) & to) || (DistanceRingBB[ksq][1] & to))
              return false;
          if (!(DistanceRingBB[square<KING>(~us)][1] & ksq))
          {
              ...
              if (!(DistanceRingBB[to][1] & square<KING>(~us)))
              {
                  Bitboard blast = DistanceRingBB[to][1] & (pieces() ^ pieces(PAWN));
                  Bitboard b = pieces() ^ ((blast | capsq) | from);
  ...
  if (is_atomic())
  {
      if (DistanceRingBB[square<KING>(~us)][1] & (type_of(pc) == KING ? to : square<KING>(us)))
          return true;
      if (capture(m) && (DistanceRingBB[square<KING>(~us)][1] & to))
          return true;
  }
#endif
  if (checkers())
...
bool Position::gives_check(Move m) const {
  ...
  if (is_atomic())
  {
      ...
      Square ksq = square<KING>(~sideToMove);
      Bitboard kingRing = DistanceRingBB[ksq][1];
...
#ifdef ATOMIC
          && !(is_atomic() && (DistanceRingBB[to - pawn_push(us)][1] & square<KING>(them)))
#endif
...
Key Position::key_after(Move m) const {
  ...
  if (captured)
  {
      ...
      if (is_atomic())
      {
          Bitboard blast = (DistanceRingBB[to][1] & (pieces() ^ pieces(PAWN))) - from;
          while (blast)
          {
              Square bsq = pop_lsb(&blast);
              Piece bpc = piece_on(bsq);
...
Value Position::see<ATOMIC_VARIANT>(Move m, PieceType nextVictim, Square s) const {
  ...
  Bitboard blast = DistanceRingBB[to_sq(m)][1] & (pieces() ^ pieces(PAWN)) & ~SquareBB[from];
  if (s != to_sq(m))
      blast &= ~SquareBB[s];
