...
PieceType pick_lva(const Bitboard* bb, Square to, Color c, Bitboard our, Bitboard& occupied) {

  Bitboard b =  Pt == BISHOP || Pt == ROOK ? attacks_bb<Pt>(to, occupied)
              : Pt == QUEEN  ? attacks_bb<ROOK>(to, occupied) | attacks_bb<BISHOP>(to, occupied)
              : Pt == PAWN   ? StepAttacksBB[make_piece(~c, Pt)][to]
                             : StepAttacksBB[Pt][to];

  b &= bb[Pt] & our & occupied;

  if (!b)
      return pick_lva<PieceType(Pt+1)>(bb, to, c, our, occupied);
...
PieceType pick_lva<PieceType(KING+1)>(const Bitboard*, Square, Color, Bitboard, Bitboard&) {
...
Value Position::see(Move m) const {
  ...
  while (true)
  {
      ...
      capturing = pick_lva<PAWN>(byTypeBB, to, stm, pieces(stm), occupied);
