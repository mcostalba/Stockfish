...
/// for the defending side in the search, this is usually sufficient to
/// win KQ vs KR.
template<>
...
/// KRP vs KR. This function knows a handful of the
...
/// KRPP vs KRP. There is just a single rule:
/// If the stronger side has no passed pawns and the defending king
...
/// KBP vs KB. There are two rules: If the defending
...
/// KBPP vs KB. It detects a few basic draws with
...
/// KBP vs KN. There is a single rule: If the defending
/// king is somewhere along the path of the pawn, and the square of the king is
...
/// KNP vs K. There is a single rule: If the pawn is a rook pawn
...
/// KNP vs KB. If knight can block bishop from taking
/// pawn, it's a win. Otherwise, draw.
template<>
...
/// KP vs KP. This is done by removing the weakest side's
/// pawn and probing the KP vs K bitbase: If the weakest side has a draw without
/// the pawn, it probably has at least a draw with the pawn as well. The exception
